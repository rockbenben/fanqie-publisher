...
清单（log_fail_list 会压成章节号区间，可直接粘贴明天补传），不静默丢弃。

本测试锁住承担"记录剩余"的纯函数 record_unprocessed，以及它与
compress_chapter_nums 的补传往返。
...
fl = []
n = fu.record_unprocessed(fl, [(80, "第八十章标题"), (81, "甲"), (82, "乙")])
...
n = fu.record_unprocessed(fl, ((m[2], m[3]) for m in matched))
check("R4 生成器消费", n == 2 and fl[1][0] == "第91章 丁", fl)

# 4b) 自定义 reason（连续失败熔断中止复用同一函数，原因文案不同）
fl = []
fu.record_unprocessed(fl, [(5, "戊")], reason="流程异常中止，未处理")
...
# 5) 补传往返：撞限章 + 剩余未处理 → log_fail_list 的章节号压缩可直接粘贴
#    模拟第 79 章撞限，80-85 未处理；期望补传号 "79-85"
fl = [("第79章 撞限章", "当日发布字数已达上限: 已到达当日发布字数上限")]
fu.record_unprocessed(fl, [(c, f"第{c}章") for c in range(80, 86)])
nums = []
for label, _ in fl:
